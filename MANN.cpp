...
bool IsWithinTradingHours(SCStudyInterfaceRef sc, int i);
...
SCSFExport scsf_AdvancedOrderFlowBot(SCStudyInterfaceRef sc)
{
    ...
    if (sc.SetDefaults)
    {
        ...
        // Add user input for minimum confirming signals (use next available index)
        sc.Input[94].Name = "Minimum Confirming Signals to Trade";
        sc.Input[94].SetInt(2);
        sc.Input[94].SetIntLimits(1, 10);
        sc.Input[94].SetDescription("Number of strategies that must agree before opening a trade");
    ...
    for (int index = loopStart; index < sc.ArraySize; ++index)
    {
        // Debug Logging
        SCString logMsg;
        logMsg.Format("Processing bar index: %d", index);
        sc.AddMessageToLog(logMsg, 0);

        // Volume at Price Data Check
        if (sc.Volume[index] <= 0)
        {
            sc.AddMessageToLog("Volume data missing for current bar.", 1);
            continue;
        }

        // Update Cumulative Delta
        float currentDelta = sc.AskVolume[index] - sc.BidVolume[index];
        float prevCumulativeDelta = (index > 0) ? sc.Subgraph[0][index - 1] : 0.0f;
        float newCumulativeDelta = prevCumulativeDelta + currentDelta;
        sc.Subgraph[0][index] = newCumulativeDelta;
        if (index >= 19) {
            float sum = 0.0f;
            for (int j = index - 19; j <= index; ++j) sum += sc.Subgraph[0][j];
            sc.Subgraph[1][index] = sum / 20.0f;
        } else {
            sc.Subgraph[1][index] = 0.0f;
        }

        // --- Absorption Signals (Subgraph[2]) ---
        TradeSignal absorptionSignal = CheckLiquidityAbsorption(sc, index);
        if (absorptionSignal.direction != 0) {
            sc.Subgraph[2][index] = (absorptionSignal.direction == 1)
                ? sc.Low[index] - sc.TickSize
                : sc.High[index] + sc.TickSize;
            sc.Subgraph[2].DataColor[index] = (absorptionSignal.direction == 1)
                ? sc.Subgraph[2].PrimaryColor
                : sc.Subgraph[2].SecondaryColor;
            // Print signal to chart (log)
            SCString msg;
            msg.Format("Absorption Signal: %s at %.2f (Bar %d)",
                (absorptionSignal.direction == 1 ? "LONG" : "SHORT"),
                absorptionSignal.entryPrice, index);
            sc.AddMessageToLog(msg, 0);
        } else {
            sc.Subgraph[2][index] = 0.0f;
        }

        // --- Iceberg Signals (Subgraph[3]) ---
        TradeSignal icebergSignal = CheckIcebergDetection(sc, index);
        if (icebergSignal.direction != 0) {
            sc.Subgraph[3][index] = (icebergSignal.direction == 1)
                ? sc.Low[index] - (2 * sc.TickSize)
                : sc.High[index] + (2 * sc.TickSize);
            sc.Subgraph[3].DataColor[index] = (icebergSignal.direction == 1)
                ? sc.Subgraph[3].PrimaryColor
                : sc.Subgraph[3].SecondaryColor;
            // Print signal to chart (log)
            SCString msg;
            msg.Format("Iceberg Signal: %s at %.2f (Bar %d)",
                (icebergSignal.direction == 1 ? "LONG" : "SHORT"),
                icebergSignal.entryPrice, index);
            sc.AddMessageToLog(msg, 0);
        } else {
            sc.Subgraph[3][index] = 0.0f;
        }

        // --- Volume Imbalance (Subgraph[4]) ---
        TradeSignal imbalanceSignal = CheckVolumeImbalance(sc, index);
        if (imbalanceSignal.direction != 0) {
            sc.Subgraph[4][index] = (imbalanceSignal.direction == 1)
                ? sc.Low[index] - (2 * sc.TickSize)
                : sc.High[index] + (2 * sc.TickSize);
            sc.Subgraph[4].DataColor[index] = (imbalanceSignal.direction == 1)
                ? sc.Subgraph[4].PrimaryColor
                : sc.Subgraph[4].SecondaryColor;
            // Print signal to chart (log)
            SCString msg;
            msg.Format("Imbalance Signal: %s at %.2f (Bar %d)",
                (imbalanceSignal.direction == 1 ? "LONG" : "SHORT"),
                imbalanceSignal.entryPrice, index);
            sc.AddMessageToLog(msg, 0);
        } else {
            sc.Subgraph[4][index] = 0.0f;
        }

        // --- Stop Run Signals (Subgraph[5]) ---
        TradeSignal stopRunSignal = CheckStopRunAnticipation(sc, index);
        if (stopRunSignal.direction != 0) {
            sc.Subgraph[5][index] = (stopRunSignal.direction == 1)
                ? sc.Low[index] - (2 * sc.TickSize)
                : sc.High[index] + (2 * sc.TickSize);
            sc.Subgraph[5].DataColor[index] = (stopRunSignal.direction == 1)
                ? sc.Subgraph[5].PrimaryColor
                : sc.Subgraph[5].SecondaryColor;
            // Print signal to chart (log)
            SCString msg;
            msg.Format("StopRun Signal: %s at %.2f (Bar %d)",
                (stopRunSignal.direction == 1 ? "LONG" : "SHORT"),
                stopRunSignal.entryPrice, index);
            sc.AddMessageToLog(msg, 0);
        } else {
            sc.Subgraph[5][index] = 0.0f;
        }

        // --- HVN Levels (Subgraph[6]) ---
        // You may want to call ProcessVolumeProfile(sc, index) here if needed
        // For now, clear by default
        sc.Subgraph[6][index] = 0.0f;
        // --- LVN Levels (Subgraph[7]) ---
        sc.Subgraph[7][index] = 0.0f;

        // --- Portfolio Heat (Subgraph[8]) ---
        // If you want to show portfolio heat per bar, call UpdateRiskMetrics with index
        // For now, clear by default
        sc.Subgraph[8][index] = 0.0f;

        // --- Strategy Signals Combined (Subgraph[9]) ---
        // Example: plot if any signal is present
        if (absorptionSignal.direction != 0 || icebergSignal.direction != 0 || imbalanceSignal.direction != 0 || stopRunSignal.direction != 0) {
            sc.Subgraph[9][index] = sc.Close[index];
            sc.Subgraph[9].DataColor[index] = sc.Subgraph[9].PrimaryColor;
        } else {
            sc.Subgraph[9][index] = 0.0f;
        }
    }
...
// Update IsWithinTradingHours to use loop variable
bool IsWithinTradingHours(SCStudyInterfaceRef sc, int i)
{
